...
router.get('/', (req, res) => {
     ...
     */
    db.query('SELECT * FROM vehicles', function (err, results, fields) {
        /**
    ...
    function render(vehicles) {
        res.render('home/views/index', { vehicles: vehicles});
    }
});
...
router.post('/new', (req, res) => {
    var db = require('../../lib/database')();
    db.query(`INSERT INTO \`vehicles\` (\`maker\`, \`model\`, \`year\`, \`platenumber\`, \`state\`)  VALUES ("${req.body.maker}", "${req.body.model}", ${req.body.year}, "${req.body.platenumber}", "${req.body.state}")`, (err, results, fields) => {
        if (err) console.log(err);
        res.redirect('/index');
    });
});
router.get('/%60:id%60', (req,res) => {
    if(req.params.id){
        var db = require('../../lib/database')();
        db.query(`SELECT * FROM vehicles WHERE id=?`,[req.params.id], (err,rows) => {
            if(err) console.log(err);
            else
                res.render('user/views/chosenvehicle', {vehicles: rows});
        });
    }
}); 
router.post('/edit/%60:id%60', (req,res)=> {
    if(req.params.id){
        var db = require('../../lib/database')();
        db.query(`SELECT * FROM vehicles WHERE id=?`,[req.params.id], (err,rows) => {
            if(err) console.log(err);
            else
                res.render('user/views/editvehicle', {vehicles: rows});
        });
    }    
});
router.post('/edited', (req,res)=>{
        var db = require('../../lib/database')();
        db.query(`UPDATE \`vehicles\` SET vehicles.maker=?, vehicles.model=?, vehicles.year=?, vehicles.platenumber=?, vehicles.state=? WHERE vehicles.id=?`,[req.body.maker, req.body.model, req.body.year, req.body.platenumber, req.body.state, Number(req.body.id)], (err, rows) => {
            if (err) console.log(err);
...
router.post('/delete', (req,res) => {
    var db = require('../../lib/database')();
    db.query(`DELETE from vehicles WHERE vehicles.id = ?`,[req.body.ID], (err, rows) => {
        if (err) console.log(err);
        res.redirect('/index');
    });
});
